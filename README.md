...
Docker Compose سرویس **Chronos‑2** و فرمان‌های پایتونی Ramon را داخل کانتینر اجرا می‌کند؛ بسته‌های Python و PyTorch داخل image نصب می‌شوند و به APT سیستم میزبان وابسته نیستند. MT5، اکسپرت MQL5 و MetaEditor در نصب فعلی کاربر همچنان خارج از Docker و زیر Wine اجرا می‌شوند. کانتینری کردن مدل، مشکل فایل گمشدهٔ MetaEditor یا کامپایل اکسپرت را رفع نمی‌کند. اجرای زنده به‌صورت پیش‌فرض در اکسپرت خاموش است.
...
برای خاموش کردن سرویس: `docker compose down`. این فرمان volume مدل را حذف نمی‌کند؛ از `down -v` استفاده نکنید مگر اینکه بخواهید cache مدل را پاک کنید. برای نصب/کامپایل EA در MT5 موجود، بخش بعدی و دستور `bash scripts/setup_local.sh --mt5-only` را ببینید؛ اگر MetaEditor وجود ندارد، ابتدا باید نصب MT5 را بررسی کنید. پس از کامپایل، اتصال اکسپرت طبق بخش «اتصال اکسپرت» انجام می‌شود.
